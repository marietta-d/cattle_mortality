...
```{r}
survival_data_without_outliers <- read.csv("./data/data_without_outliers.csv")
...
```{r}
colnames(breed_wide_data[, 90:184])
```
...
```
Plot the curves for top 8 breeds
```{r}
N <- 8
top_8_breeds <- survival_data_without_outliers %>%
  count(Breed, sort=TRUE) %>%
  top_n(N, n) %>%
  pull(Breed)


filtered_top_8_breeds <- survival_data_without_outliers %>%
  filter(Breed %in% top_8_breeds)
...
Top_8_breeds_fit_years <- survfit(Surv(years_alive, status) ~ Breed, data=filtered_top_8_breeds)
...
summary(Top_8_breeds_fit_years)$table

```

```{r}
survival_top_8_breeds_years <- data.frame(time = Top_8_breeds_fit_years$time,
                  n.risk = Top_8_breeds_fit_years$n.risk,
                  n.event = Top_8_breeds_fit_years$n.event,
                  n.censor = Top_8_breeds_fit_years$n.censor,
                  surv = Top_8_breeds_fit_years$surv,
                  upper = Top_8_breeds_fit_years$upper,
                  lower = Top_8_breeds_fit_years$lower
                  )
survival_top_8_breeds_years
```


```{r}
ggsurvplot(Top_8_breeds_fit_years,
          pval = TRUE, conf.int = TRUE,
          risk.table = TRUE, # Add risk table
...
Dairy:
```{r}
top_dairy_breeds <- filtered_top_8_breeds %>%
...
ggsave("km_top_dairy_breed_years.png", plot=km_top_dairy_breed_years$plot, width = 8, height = 6)
```

Breed by production type survival curve
Beef:
```{r}
top_beef_breeds <- filtered_top_8_breeds %>%
...
km_top_beef_breeds_years
ggsave("km_top_beef_breeds_years.png", plot=km_top_beef_breeds_years$plot, width = 8, height = 6)
```
...
N <- 4
top_4_beef_breeds <- top_beef_breeds %>%
  count(Breed, sort=TRUE) %>%
  top_n(N, n) %>%
  pull(Breed)


filtered_top_4_beef_breeds <- top_beef_breeds %>%
  filter(Breed %in% top_4_beef_breeds)

top_4_beef_breeds_fit_years <- survfit(Surv(years_alive, status) ~ Breed, data=filtered_top_4_beef_breeds)

km_top_4_beef_breeds_years <- ggsurvplot(top_4_beef_breeds_fit_years,
          pval = TRUE, conf.int = TRUE,
          risk.table = TRUE, # Add risk table
          risk.table.col = "Breed", # Change risk table color by groups
          #linetype = "Breed", # Change line type by groups
...
km_top_4_beef_breeds_years
ggsave("km_top_4_beef_breeds_years.png", plot=km_top_4_beef_breeds_years$plot, width = 8, height = 6)
```
