...
```{r}
not_sign <- variables[c("NAD", "PLEUR.PNEUMONIA.GENERAL", "HAEMATOMA", "TB.GENERALISED", "Ballymena", "Mallusk", "ab_7a032bbcfa7447fb21edc6e6f8d7bab0bed1bb16", "ab_52d480596328cca7948b6f1f31e1f2b18590cf10", "ab_432d3a852c6074754d640e8d1369a1c56b0d5805", "AA", "LIM", "BB", "CH", "HER", "SIM")]

variables <- variables[-c(10, 23, 27, 29, 33, 38, 53, 57, 55, 59, 60, 62, 63, 64, 65)]
variables 
```

```{r}
colSums(train_data[-1])
train_data
```
...
Modelling attempt with all variables except those without observations
```{r}
...
Modelling attempt only sign.variables from univariate modelling
```{r}
cox_multi_signif <- coxph(Surv(months_alive, status) ~  Total_Conditions_per_Animal + T_2M + RH2M + No + TB + FACTORY.DAMAGE +
                                  FASCIOLIASIS + PLEURISY + NEPHRITIS + SCAR.TISSUE + INJECTION.SITES + ABSCESS.PYAEMIA +
                                  CONTAMINATION + ACTINO + BRUISING + HYDRONEPHROSIS + TELANGECTASIS + PETECHAE.HAEMORRHAGES +
                                   PERICARDITIS + FEVERED + PERITONITIS + PLEUR.PNEUMONIA.LOCAL + 
                                   RESIDUES + ARTHRITIS + OEDEMA + EMACIATION + Armagh + 
                                  Ballymena + Coleraine + Dungannon + Enniskillen + LondonDerry + Mallusk + Newry + NtArds + 
                                  Omagh + ab_4f71277e271f09349a4d6d5f6c05acf635652015 + ab_6f7d51dd8564e05e279faa791f97894ef141a4db +
                                  ab_32d8a591f643abd1ff6a998b128403c8962a55d8 + ab_f833f5d69610136a115182502764c803be97ee16 + 
                                  ab_94003c4910d6c601a698a3258918fc822b88dd0d + ab_332a6a1d2843f367c160a1c7c3fb2a28f0f8909e + 
                                  ...
                                  ab_ba747dbffafe62fb8bea541fd72df6d8c7afcb2f + FR + HOL + F + B + M + died_on_abattoir + died_on_farm, 
                   data = train_data)
summary(cox_multi_signif)
```

Backward elimination for all significant variables after eliminate Omagh, HOL, M, and Died_on_farm
```{r}
selectedSupermodel_only_sign <- selectCox(Surv(train_data$months_alive, train_data$status) ~ 
                                  Total_Conditions_per_Animal + T_2M + RH2M + No + TB + FACTORY.DAMAGE +
                                  FASCIOLIASIS + PLEURISY + NEPHRITIS + SCAR.TISSUE + INJECTION.SITES + ABSCESS.PYAEMIA +
                                  CONTAMINATION + ACTINO + BRUISING + HYDRONEPHROSIS + TELANGECTASIS + PETECHAE.HAEMORRHAGES +
                                   PERICARDITIS + FEVERED + PERITONITIS + PLEUR.PNEUMONIA.LOCAL + 
...
```{r}
simple_cox <- coxph(Surv(train_data$months_alive, train_data$status) ~ F + TB, data=train_data)
simple_cox_train <- survfit(simple_cox, data=train_data)
ggsurvplot(simple_cox_train, ggtheme = theme_minimal()) 
```
...
visualise the selected supermodel
```{r}
super_cox_train <- survfit(fit_superCox, data=train_data)
super_cox_train
ggsurvplot(super_cox_train, ggtheme = theme_minimal(), variable="TB")
```

```{r}
fit_superCox
survival <- data.frame(survival = seq(.01, .99, by = .01))
survival
super_cox_predictions <- data.frame(survival = survival,
                                    predictions = predict(fit_superCox, survival))
