...
Load dataset 
```{r}
survival_data <- read.csv("data_wide.csv")
```
...
Delete rows where the Sex is "U" (delete 8 rows)
```{r}
...
If we take the 1.5*IQR the data considered outliers are 290063.
Animals over 7 years old consider outliers, 17% of the female animals and 19% of the Dairy animals 
```{r}
...
outliers1_info <- summary(survival_data[outliers1, c(6, 10, 11, 15)])
outliers1_info
```


```{r}
paste("proportion of beef animals consider outliers = ", 144027/No_Beef)
paste("proportion of Dairy animals consider outliers = ", 146036/No_Dairy)

paste("proportion of Male animals consider outliers = ", 302/No_Male)
paste("proportion of Female animals consider outliers = ", 285243/No_Female)
paste("proportion of Bulls consider outliers = ", 4518/No_Bull)
```
...
```{r}
paste("proportion of beef animals consider outliers = ", 73826/No_Beef)
paste("proportion of Dairy animals consider outliers = ", 34204/No_Dairy)

paste("proportion of Male animals consider outliers = ", 55/No_Male)
paste("proportion of Female animals consider outliers = ", 106629/No_Female)
paste("proportion of Bulls consider outliers = ", 1346/No_Bull)
```
...
```{r}
# 2018
data_2018 <- filter(survival_data, format(Dod, "%Y") == "2018")
...
# 2019
data_2019 <- filter(survival_data, format(Dod, "%Y") == "2019")
...
# 2020
data_2020 <- filter(survival_data, format(Dod, "%Y") == "2020")
...
# 2021
data_2021 <- filter(survival_data, format(Dod, "%Y") == "2021")
...
# 2022
data_2022 <- filter(survival_data, format(Dod, "%Y") == "2022")
...
```{r}
table(data_without_outliers$Dvo_Code, data_without_outliers$Beef_or_Dairy)
#dfxyz$zxc <- dfxyz$
```
...
While 23 breeds have less than 10 animals.
...
```{r}
long_survival_data <- read.csv("data_long.csv")
```


Change the type of variables
```{r}
long_survival_data[,c("Dob", "Dod")] = lapply(survival_data[,c("Dob", "Dod")],
                                function(x) as.Date(x))
```
...
Delete rows where the Sex is "U" (delete 8 rows)
```{r}
long_survival_data <- long_survival_data %>%
  filter(Sex!="U")
```
...
dist_health_conditions.png <- ggplot(filtered_conditions_without_zero, aes(x=reorder(ConditionsbyAI,ConditionsbyAI,
...
```{r}
write.csv(data_without_outliers, file="data_without_outliers.csv")
