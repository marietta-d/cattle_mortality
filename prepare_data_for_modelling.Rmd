...
conditions_columns <- colnames(survival_data_without_outliers[21:92])
conditions_columns
...
rare_conditions_data <- filtered_rare_conditions %>%
  filter(Total_Conditions_per_Animal > 0 & rowSums(filtered_rare_conditions[21:44]) == 0) 
common_conditions_data <- filtered_rare_conditions %>%
  filter(Total_Conditions_per_Animal== 0 | rowSums(filtered_rare_conditions[21:44]) >0 ) 
  
```
I will delete 2693 entries where the animals don't have any of the common conditions
