...
years
```{r}
baseline_survival_fit <- survfit(Surv(years_alive, status) ~ 1, data = survival_data)
...
baseline_survival_fit_days <- survfit(Surv(days_alive, status) ~ 1, data = survival_data)

summary(baseline_survival_fit_days)$table
...
years
```{r}
sex_fit_years <- survfit(Surv(years_alive, status) ~ Sex, data=survival_data)
```
...
sex_fit_days <- survfit(Surv(days_alive, status) ~ Sex, data=survival_data)
