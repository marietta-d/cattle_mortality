...
merged_data$Herd_Id <- as.character(merged_data$Herd_Id)
```
...
herd_id_old_data <- merged_data$Herd_Id
...
```{r}
unique(sort(herd_id_size))
```


```{r}
unique(sort(herd_id_old_data))
```


### I dont have the same herd Id's in the two datasets ###
...
merged_wide <- long_merged_data %>%
  mutate(ConditionPresent = 1) %>%
  pivot_wider(names_from = ConditionsbyAI, values_from = ConditionPresent, values_fill = list(ConditionPresent = 0))

merged_wide[merged_wide$Animal_Id=="5dd425da1403654d3dff7ea12ff3e1d1d9f82af2", ]
merged_data[merged_data$Animal_Id=="5dd425da1403654d3dff7ea12ff3e1d1d9f82af2", ]
long_merged_data[long_merged_data$Animal_Id=="5dd425da1403654d3dff7ea12ff3e1d1d9f82af2", ]
...
```{r}
write.csv(merged_wide, file="./data/merged_weather_wide.csv", row.names = FALSE)
```
