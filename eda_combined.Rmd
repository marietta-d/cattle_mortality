...
avd <- aggregate(days_alive ~ Sex + Beef_or_Dairy, data = combined_data_without_outliers, mean)
avd
```

```{r}
healthy_vs_unhealthy <- avd %>%
  mutate(days_alive_unhealthy = mean_age_healthy_animals$days_alive, difference = days_alive-days_alive_unhealthy)
healthy_vs_unhealthy
```

Healthy animals live less than the unhealthy ones the biggest differences is for beef bulls(94 days) and dairy female(82 days).
