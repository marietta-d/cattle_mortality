...
Load dataset 
```{r}
combined_data <- read.csv("./data/final_merged_wide.csv") # this dataset contains na at the herd size correct dataset on the one drive
head(combined_data)
combined_size_weather <- read.csv("./data/final_merged_wide.csv")
combined_size_weather[combined_size_weather$Herd_Id=="fef07ff7ccf1f32b2bb31f90a8eee5c5d9ceb639", ]
```
...
```{r}
sum(is.na(combined_data_without_outliers$AvgOftotal_animals))
```
...
The vertical lines are not correct (red is median)
```{r}
...
days_alive_histogram_no_outliers <- ggplot(combined_data_without_outliers, aes(x=days_alive, after_stat(density))) +
  ...
  geom_vline(aes(xintercept= mean(days_alive), color="red")) +
  geom_vline(aes(xintercept=median(days_alive), color = "green"))+
  labs(title="Histogram of age in days", x="days", y="Animals")+
  theme_minimal()
...
Herd_freq_table <- combined_data_without_outliers %>%
  ...
  dplyr::summarise(frequency = n(), 
                   age_days= mean(days_alive), 
                   age_years=mean(years_alive),
                   mean_size = mean(AvgOftotal_animals)) %>%
  arrange(desc(frequency))
Herd_freq_table
...
```{r}
summary(combined_data_without_outliers$AvgOftotal_animals)
```
...
### Numerical Variables
```{r}
colSums(is.na(combined_data_without_outliers))
...
```{r}
# how many herd_ids have missing values?
str(combined_data_without_outliers)
not_missing_herd_size <- drop_na(combined_data_without_outliers)
str(not_missing_herd_size)
not_missing_herd_size <- droplevels(not_missing_herd_size)
str(not_missing_herd_size)
list_of_herd_id <- levels(combined_data_without_outliers$Herd_Id)
list_of_not_mis_herd_size <- levels(not_missing_herd_size$Herd_Id)
list_of_herds_missing_valus <- setdiff(list_of_herd_id, list_of_not_mis_herd_size)
list_of_herds_missing_valus
```

```{r}
herd_size_missing_values <- combined_data_without_outliers %>%
  filter(is.na(combined_data_without_outliers$AvgOftotal_animals))
write.csv(herd_size_missing_values, file="./data/herd_size_missing_values.csv", row.names = FALSE)
```



```{r}
missing_herd_size <- data.frame()
for(i in 1:length(list_of_herds_missing_valus)){
  id <- list_of_herds_missing_valus[i]
  df_i <- data.frame(combined_data_without_outliers[combined_data_without_outliers$Herd_Id==id, ])
missing_herd_size <- rbind(missing_herd_size, df_i)
}

missing_herd_size
droplevels(missing_herd_size)
str(missing_herd_size)
```


```{r}
         
herd_size_hist <- ggplot(not_missing_herd_size, aes(x=AvgOftotal_animals)) +
  geom_histogram(binwidth=0.5, fill="steelblue") +
  geom_vline(aes(xintercept= mean(AvgOftotal_animals), color="red")) +
  labs(title="Average herd size", x="Herd size", y="Animals") +
  theme(axis.text.x = element_text(color = "grey20", size = 16, angle = 0, hjust = .5, vjust = .5, face = "plain"),
        ...
        plot.background = element_rect(fill = "transparent", color = NA)) 
herd_size_hist
ggsave("./data/combined_figs/herd_size_hist.png", plot=herd_size_hist, width=8, height = 6)
```


```{r}
herd_size_scatter_plot <- ggplot(not_missing_herd_size, aes(x=AvgOftotal_animals, y=days_alive)) + geom_point()
ggsave("./data/combined_figs/herd_size_scatter_plot.png", plot=herd_size_scatter_plot, width=8, height = 6)
herd_size_scatter_plot
```
...
```{r}
colnames(unhealthy_data)
conditions_list <- colnames(unhealthy_data[21:41])
...
for (i in 1:num_conditions) {
  ...
  condition_slice <- unhealthy_data %>%
    filter(unhealthy_data[condition]>0) %>%
    dplyr::summarise(animals = n(),
              mean_age = mean(days_alive),
              prcnt_male = mean(Sex == "M") * 100, 
              prcnt_female = mean(Sex == "F") * 100, 
              prcnt_bull = mean(Sex == "B") * 100, 
              prcnt_beef = mean(prod_type == "Beef") * 100, 
              prcnt_dairy = mean(prod_type == "Dairy") * 100, 
              prcnt_abattoir = mean(place_of_death == "Abattoir") * 100, 
              prcnt_farm = mean(place_of_death == "Died on Farm") * 100)
...
summary(unhealthy_data$days_alive)
```
...
save combined_data_without_outliers
```{r}
write.csv(combined_data_without_outliers, file="./data/new_combined_data_without_outliers.csv", row.names = FALSE)
```
